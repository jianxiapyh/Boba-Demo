...
class Demo2ServerApiTest(unittest.TestCase):
    ...
    def test_default_phone_stream_and_control_offset(self):
        ...
        self.assertEqual(args.phone_stream_max_fps, 0.0)
        self.assertEqual(
            build_parser().parse_args(["--phone_stream_fps", "12"]).phone_stream_max_fps,
            12.0,
        )
        self.assertEqual(args.phone_stream_encode_workers, 2)
        self.assertEqual(args.phone_stream_settle_motion, 1e-4)
        self.assertEqual(args.phone_stream_settle_frames, 5)
        self.assertEqual(args.phone_stream_full_rate_settle_s, 1.0)
        self.assertEqual(args.phone_stream_recovery_fps, 2.0)
        ...
        self.assertEqual(args.qr_size, 320)
        self.assertFalse(args.travel_router)
    ...
    def test_phone_client_prefers_acknowledged_websocket_with_mjpeg_fallback(self):
        html = load_phone_client_html()

        self.assertIn("new WebSocket(socketUrl)", html)
        self.assertIn('"https:" ? "wss" : "ws"', html)
        self.assertIn('socket.send("next")', html)
        self.assertIn('streamTransport = "websocket"', html)
        self.assertIn('streamTransport = "mjpeg"', html)
        self.assertIn("function useMjpegFallback(", html)

    def test_phone_input_is_coalesced_instead_of_posted_every_frame(self):
        html = load_phone_client_html()

        self.assertIn("function queueInputSend()", html)
        self.assertIn("async function flushInput()", html)
        self.assertIn("inputSendRunning", html)
        self.assertNotIn("setInterval(sendInput, 33)", html)

    def test_demo2_phone_stream_resize_is_batched_and_demand_driven(self):
        ...
        self.assertIn("stream_session_ids_fn(occupied_sessions)", publish_block)
        self.assertIn("stream_activity_policy.eligible_sessions(", publish_block)
        self.assertIn("stream_activity_policy.mark_published(", publish_block)
        self.assertIn("for idx, session_id in enumerate(stream_session_ids):", publish_block)
        self.assertNotIn("tile = batch_frames[session_id]", publish_block)
        self.assertNotIn("unsqueeze(0)", publish_block)

    def test_phone_stream_settling_uses_batched_gpu_object_motion(self):
        trainer_source = Path(REPO_ROOT / "qqtt" / "engine" / "trainer_warp.py").read_text()

        self.assertIn("motion_session_ids = stream_activity_policy.motion_session_ids()", trainer_source)
        self.assertIn("current_object_positions[motion_ids]", trainer_source)
        self.assertIn("previous_object_positions[motion_ids]", trainer_source)
        self.assertIn("torch.mean(torch.sum(displacement.square(), dim=2), dim=1)", trainer_source)
        self.assertNotIn("for session_id in motion_session_ids", trainer_source)

    def test_public_monitor_shows_measured_aggregate_throughput(self):
        trainer_source = Path(REPO_ROOT / "qqtt" / "engine" / "trainer_warp.py").read_text()
        self.assertIn("AggregateThroughputMeter(", trainer_source)
        self.assertIn('label = "AGG THROUGHPUT"', trainer_source)
