...
def build_rotation_reuse_cache(
    ...
    R_cache = torch.eye(3, device=device, dtype=torch.float32).repeat(object_mass_node_total, 1, 1).contiguous()
    ...
    xyz_local_w = (xyz_local * weights.unsqueeze(-1)).contiguous()  # (Ng, K, 3)
    ...
    Q_cache_bm = torch.zeros(mass_node_per_instance, number_of_instance, 4, device=device, dtype=torch.float32)
    Q_cache_bm[..., 0] = 1.0
    ...
    def build_W_csr(weights_indices_ik: torch.Tensor, weights_ik: torch.Tensor, Nb_: int) -> torch.Tensor:
        ...
        val = weights_ik.reshape(-1).to(device=device, dtype=torch.float32)                                                     # (Ng*K,)
    ...
    gs_rest = Func.normalize(gaussians_quat_rest, dim=-1, eps=1e-12).contiguous()
    rw, rx, ry, rz = gs_rest.unbind(-1) 
    rest_right_mat_T = torch.stack([
        torch.stack([ rw,  rx,  ry,  rz], dim=-1),
        torch.stack([-rx,  rw, -rz,  ry], dim=-1),
        torch.stack([-ry,  rz,  rw, -rx], dim=-1),
        torch.stack([-rz, -ry,  rx,  rw], dim=-1),
    ], dim=-2).contiguous()  # (Ng,4,4)
    ...
    cache = {
        ...
        "rest_right_mat_T": rest_right_mat_T,  # (N,4,4) Precomputed rest right-multiplication matrices
...
def lbs_with_rotation_reuse(
    ...
    if bones_to_recompute.numel() > 0:
        ...
        Q_new = rotmat_to_quat_fast(R_computed)  
        ...
        Q_flat = cache["Q_cache_bm"].reshape(mass_nodes_per_instance * number_of_instance, 4)  # contiguous view
        Q_old = Q_flat.index_select(0, idx)
        flip = (Q_new * Q_old).sum(dim=-1, keepdim=True) < 0
        Q_new = torch.where(flip, -Q_new, Q_new)
        # write back
        Q_flat.index_copy_(0, idx, Q_new)
        
        # Update caches 
        R_cache.index_copy_(0, bones_to_recompute, R_computed)
    ...
    # Rotate by bone rotations
    # R_per_gaussian: (I, Ng, K, 3, 3)
    # xyz_local:      (Ng, K, 3)
    ...
    cache["motions_bm"].copy_(motions.transpose(0, 1))  # (Nb,I,3)
    
    M_mat = cache["motions_bm"].reshape(mass_nodes_per_instance, number_of_instance * 3)         # view
    M_out = torch.sparse.mm(cache["W_csr"], M_mat)                      # (Ng, I*3)
    ...
    quat_deformed = None
    if gaussians_quat_rest is not None:
        Q_mat = cache["Q_cache_bm"].reshape(mass_nodes_per_instance, number_of_instance * 4)   # view, no copy
        Q_out = torch.sparse.mm(cache["W_csr"], Q_mat)                                     # (Ng, I*4)
        q = Q_out.view(gaussians_per_instance, number_of_instance, 4) 
        tmp_new = torch.matmul(q, cache["rest_right_mat_T"])
        quat_def_nic = Func.normalize(tmp_new, dim=-1, eps=1e-12)       # (Ng,I,4)
