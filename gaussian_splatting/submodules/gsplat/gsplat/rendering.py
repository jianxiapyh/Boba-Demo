...
def _normalize_shared_template_backgrounds(
    ...
    if backgrounds.dim() == 1:
        assert (
            num_cameras == 1 and backgrounds.shape[0] == num_channels
        ), backgrounds.shape
        return backgrounds.unsqueeze(0)
    assert backgrounds.shape == (num_cameras, num_channels), backgrounds.shape
    return backgrounds


def _gather_shared_template_colors(
    colors: Tensor,
    template_ids: Tensor,
    global_ids: Tensor,
    camera_ids: Tensor,
    ...
    if colors.dim() == 2:
        if colors.shape[0] == gaussians_per_instance:
            return colors[template_ids]
        if colors.shape[0] == total_gaussians:
            return colors[global_ids]
    elif colors.dim() == 3:
        if colors.shape[:2] == (num_cameras, gaussians_per_instance):
            return colors[camera_ids, template_ids]
        if colors.shape[:2] == (num_cameras, total_gaussians):
            return colors[camera_ids, global_ids]
    raise ValueError(
        "Unsupported shared-template color shape: "
        f"{tuple(colors.shape)} for total_gaussians={total_gaussians}, "
        f"gaussians_per_instance={gaussians_per_instance}, num_cameras={num_cameras}"
    )


def _gather_shared_template_shs(
    colors: Tensor,
    template_ids: Tensor,
    global_ids: Tensor,
    camera_ids: Tensor,
    ...
    if colors.dim() == 3:
        ...
        if colors.shape[:2] == (num_cameras, total_gaussians):
            return colors[camera_ids, global_ids]
    raise ValueError(
        "Unsupported shared-template SH shape: "
        f"{tuple(colors.shape)} for total_gaussians={total_gaussians}, "
        f"gaussians_per_instance={gaussians_per_instance}, num_cameras={num_cameras}"
    )


def rasterization_shared_template(
    means: Tensor,  # [B * G, 3]
    quats: Tensor,  # [B * G, 4]
    scales: Tensor,  # [G, 3]
    opacities: Tensor,  # [G]
    colors: Tensor,  # [G, D] / [G, K, 3] / full-length equivalents
    gaussians_per_instance: int,
    viewmats: Tensor,  # [C, 4, 4]
    Ks: Tensor,  # [M, 3, 3]
    width: int,
    height: int,
    intrinsic_ids: Optional[Tensor] = None,  # [C] or None
    ...
    batch_image_mode: bool = False,
    fuse_shared_template_sh: bool = True,
    ellipse_tile_filter: bool = True,
    profiler=None,
) -> Tuple[Tensor, Tensor, Dict]:
    """Rasterize batched gaussians while keeping static template parameters shared."""

    assert means.dim() == 2 and means.shape[-1] == 3, means.shape
    assert quats.shape == (means.shape[0], 4), quats.shape
    assert scales.shape == (gaussians_per_instance, 3), scales.shape
    ...
    if total_gaussians % gaussians_per_instance != 0:
        raise ValueError(
            "means length must be divisible by gaussians_per_instance, got "
            f"{total_gaussians} and {gaussians_per_instance}"
        )

    num_instances = total_gaussians // gaussians_per_instance
    num_cameras = viewmats.shape[0]
    num_output_images = num_instances * num_cameras if batch_image_mode else num_cameras
    device = means.device

    if opacities.dim() == 2 and opacities.shape == (gaussians_per_instance, 1):
        opacities = opacities.squeeze(-1)
    assert opacities.shape == (gaussians_per_instance,), opacities.shape

    calc_compensations = rasterize_mode == "antialiased"
    campos = (
        torch.inverse(viewmats)[..., :3, 3] if sh_degree is not None else None
    )  # [C, 3]
    use_fused_shared_template_sh = (
        fuse_shared_template_sh
        and batch_image_mode
        and num_cameras == 1
        and sh_degree == 3
        and render_mode in ["RGB", "RGB+D", "RGB+ED"]
        and colors.dim() == 3
        and tuple(colors.shape) == (gaussians_per_instance, 16, 3)
    )

    means_batched = means.reshape(num_instances, gaussians_per_instance, 3)
    quats_batched = quats.reshape(num_instances, gaussians_per_instance, 4)

    with _profile(profiler, "fully_fused_projection_ms"):
        if use_fused_shared_template_sh:
            (
                batch_ids,
                camera_ids,
                gaussian_ids,
    ...
    backgrounds = _normalize_shared_template_backgrounds(
        backgrounds, num_cameras, base_channels
    )
    ...
    if radii.numel() == 0:
        render_colors = torch.zeros(
            (num_output_images, height, width, output_channels),
            device=device,
            dtype=means.dtype,
        )
        render_alphas = torch.zeros(
            (num_output_images, height, width, 1),
            device=device,
            dtype=torch.float32,
        )
        if backgrounds is not None:
            background_fill = backgrounds
            if render_mode in ["RGB+D", "RGB+ED"]:
                background_fill = torch.cat(
                    (
                        backgrounds,
                        torch.zeros(
                            (num_cameras, 1),
                ...
                background_fill = torch.zeros(
                    (num_cameras, 1),
                    device=backgrounds.device,
                    dtype=backgrounds.dtype,
                )
            if batch_image_mode:
                background_fill = background_fill.repeat(num_instances, 1)
            render_colors = background_fill[:, None, None, :].expand(
                -1, height, width, -1
            ).clone()
        empty_long = torch.empty((0,), device=device, dtype=torch.long)
        empty_radii = torch.empty((0, 2), device=device, dtype=torch.int32)
        empty_means2d = torch.empty((0, 2), device=device, dtype=means.dtype)
        empty_depths = torch.empty((0,), device=device, dtype=means.dtype)
        empty_conics = torch.empty((0, 3), device=device, dtype=means.dtype)
        meta = {
            "batch_ids": empty_long,
            "camera_ids": empty_long,
            "gaussian_ids": empty_long,
            "radii": empty_radii,
            "means2d": empty_means2d,
            "depths": empty_depths,
            "conics": empty_conics,
            "opacities": empty_depths,
            "tile_width": tile_width,
            "tile_height": tile_height,
            "tiles_per_gauss": torch.empty((0,), device=device, dtype=torch.int32),
            "isect_ids": empty_long,
            "flatten_ids": torch.empty((0,), device=device, dtype=torch.int32),
            "isect_offsets": torch.zeros(
                (num_output_images, tile_height, tile_width),
                device=device,
                dtype=torch.int32,
            ),
            "width": width,
            "height": height,
            "tile_size": tile_size,
            "n_batches": num_instances,
            "n_cameras": num_cameras,
            "n_output_images": num_output_images,
            "batch_image_mode": batch_image_mode,
        }
        if batch_image_mode:
            render_colors = render_colors.reshape(
                num_instances, num_cameras, height, width, output_channels
            )
            render_alphas = render_alphas.reshape(
                num_instances, num_cameras, height, width, 1
            )
        return render_colors, render_alphas, meta

    if not use_fused_shared_template_sh:
        with _profile(profiler, "shared_template_gather_ms"):
            batch_ids = batch_ids.to(torch.long)
            camera_ids = camera_ids.to(torch.long)
            template_ids = template_ids.to(torch.long)
            gaussian_ids = batch_ids * gaussians_per_instance + template_ids

            packed_opacities = opacities[template_ids]
            if compensations is not None:
                packed_opacities = packed_opacities * compensations

            if sh_degree is None:
                packed_colors = _gather_shared_template_colors(
                    colors=colors,
                    template_ids=template_ids,
                    global_ids=gaussian_ids,
                ...
                shs = _gather_shared_template_shs(
                    colors=colors,
                    template_ids=template_ids,
                    global_ids=gaussian_ids,
                    camera_ids=camera_ids,
                ...
                dirs = means[gaussian_ids] - campos[camera_ids]
                masks = (radii > 0).all(dim=-1)
                with _profile(profiler, "spherical_harmonics_ms"):
                    packed_colors = spherical_harmonics(
    ...
    if batch_image_mode:
        backgrounds_for_output = (
            backgrounds.repeat(num_instances, 1) if backgrounds is not None else None
        )
        image_ids = batch_ids * num_cameras + camera_ids
    else:
        backgrounds_for_output = backgrounds
        image_ids = camera_ids

    with _profile(profiler, "isect_tiles_ms"):
        tiles_per_gauss, isect_ids, flatten_ids = isect_tiles(
            ...
            image_width=width,
            image_height=height,
            ellipse_tile_filter=ellipse_tile_filter and batch_image_mode,
            profiler=profiler,
        )
    with _profile(profiler, "isect_offset_encode_ms"):
        isect_offsets = isect_offset_encode(
            isect_ids, num_output_images, tile_width, tile_height
        ).reshape(num_output_images, tile_height, tile_width)

    rasterizer_backgrounds = (
        backgrounds_for_output
        if batch_image_mode and render_mode == "RGB" and backgrounds_for_output is not None
        else None
    )

    with _profile(profiler, "rasterize_to_pixels_ms"):
        render_colors, render_alphas = rasterize_to_pixels_inference(
            means2d,
            conics,
            packed_colors,
            packed_opacities,
            ...
            isect_offsets,
            flatten_ids,
            backgrounds=rasterizer_backgrounds,
            packed=True,
        )

    with _profile(profiler, "background_depth_finalize_ms"):
        if backgrounds_for_output is not None and rasterizer_backgrounds is None:
            render_colors = render_colors + backgrounds_for_output[:, None, None, :] * (
                1.0 - render_alphas
            )
        ...
        if batch_image_mode:
            render_colors = render_colors.reshape(
                num_instances, num_cameras, height, width, render_colors.shape[-1]
            )
            render_alphas = render_alphas.reshape(
                num_instances, num_cameras, height, width, 1
            )
